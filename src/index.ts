...
export function fillParentContainer(
	...
	const { fontFamily, lineHeightRatio, maxLineHeight } = fontInfo(
		element,
		explicitLineHeight,
	);
	...
	const optimalSize = optimalFontSize(widthHeightRatios, algoParams);
...
function optimalFontSize(
	...
	const { minFontSize, maxHeight, maxFontSize, maxLineHeight, maxWidth } = args;
...
function checkConstraints(
	...
	const { maxHeight } = args;
	const height = estimateHeight(wordRatios, args);
